...
private struct ModuleHeader<Trailing: View>: View {
    let module: AsterModule
...
private struct CanvasModuleView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 18) {
                CanvasLibrary(showImporter: $showImporter)
                CanvasInspector()
                    .frame(width: 300)
            }
            .padding(22)
        ...
        VStack(spacing: 14) {
            ...
            if library.filteredItems.isEmpty {
                ContentUnavailableView {
                    ...
                    Text("Add an image or video to create your first wallpaper.")
                } actions: {
                ...
                ScrollView {
                    LazyVGrid(columns: columns, spacing: 14) {
                        ForEach(library.filteredItems) { item in WallpaperCard(item: item) }
                    }
                    .padding(1)
                }
            }
...
private struct WallpaperCard: View {
    ...
    var body: some View {
        ...
        .onTapGesture { library.selectedID = item.id }
        .contextMenu {
            Button("Set as Desktop") { controller.apply(item, url: library.url(for: item)) }
            Button("Set as Lock Screen") { controller.configureLockScreen(item, url: library.url(for: item)) }
                .disabled(item.kind != .image || item.isGIF)
...
private struct CanvasInspector: View {
    ...
    var body: some View {
        ...
        ScrollView(.vertical, showsIndicators: false) {
            ...
            DisclosureGroup("Lock Screen details") {
                VStack(alignment: .leading, spacing: 9) {
                    Text("macOS only allows a still image on the secure Lock Screen. Videos and GIFs cannot be selected. The still is stored in the system desktop layer underneath Aster’s motion wallpaper.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
    ...
    private var destinationHelp: String {
        ...
        case .lockScreen: "Still images only. macOS does not allow video on the secure Lock Screen."
        case .screenSaver: "Shown while your Mac is idle before it locks."
        }
    }
    ...
    private var actionLabel: String {
        ...
        case .lockScreen: lockSelectionIsMotion ? "Still Images Only" : "Set Lock Screen"
    ...
    private var lockSelectionIsMotion: Bool {
        ...
        return item.kind != .image || item.isGIF
    }
}
                ...
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    EditorSection(title: "Arrange real app icons", subtitle: "macOS saves the order after you move an icon.") {
                        ...
                        Text("Wi-Fi, battery, and other status icons work too—Command-drag each one to the divider’s left.")
                            ...
                            .foregroundStyle(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
                        Button("Reset Aster control positions") {
