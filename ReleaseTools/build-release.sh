...
VERSION="${ASTER_VERSION:-1.0.0-beta.6}"
BUILD="${ASTER_BUILD:-6}"
...
if [[ "$RELEASE_MODE" != production && "$RELEASE_MODE" != community && "$RELEASE_MODE" != test ]]; then
    print -u2 "ASTER_RELEASE_MODE must be 'production', 'community', or 'test'."
    exit 1
fi
...
[[ -s "$ROOT/FIRST-LAUNCH.md" ]] || {
    print -u2 "FIRST-LAUNCH.md is required in every distributable DMG."
    exit 1
}
...
ICON_SOURCE="$ROOT/dist/icon-source/AsterIcon.png"
...
if [[ "$RELEASE_MODE" == production ]]; then
    ...
    if [[ "$RELEASE_MODE" == community ]]; then
        print -u2 "Creating an ad-hoc community release. Apple has not signed or notarized this build."
    else
...
cp "$ROOT/FIRST-LAUNCH.md" "$DMG_STAGE/First Launch Instructions.txt"
...
if [[ "$RELEASE_MODE" == test ]]; then
    print "TEST ARTIFACT ONLY — rebuild in community or production mode before publishing."
elif [[ "$RELEASE_MODE" == community ]]; then
    print "COMMUNITY RELEASE — not signed or notarized by Apple. Include the first-launch instructions when publishing."
fi
